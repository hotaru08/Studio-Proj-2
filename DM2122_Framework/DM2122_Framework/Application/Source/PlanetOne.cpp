...
void PlanetOne::Update(double dt)
{
...
void PlanetOne::RenderSkyBoxNight()
{
	//sky box
	modelStack.PushMatrix();
	modelStack.Translate(camera.position.x, camera.position.y, camera.position.z);
...
void PlanetOne::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void PlanetOne::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)//x = translate by x;
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void PlanetOne::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void PlanetOne::RenderMeshOnScreen(Mesh* mesh, int x, int y, int sizex, int sizey)
{
	glDisable(GL_DEPTH_TEST);
