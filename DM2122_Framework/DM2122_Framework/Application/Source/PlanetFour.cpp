...
void PlanetFour::RenderSkyBoxNight()
{
	//sky box
	modelStack.PushMatrix();
	modelStack.Translate(camera.position.x, camera.position.y, camera.position.z);
...
void PlanetFour::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void PlanetFour::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)//x = translate by x;
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void PlanetFour::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void PlanetFour::RenderMeshOnScreen(Mesh* mesh, int x, int y, int sizex, int sizey)
{
	glDisable(GL_DEPTH_TEST);
